    ...
    */
    NodeFactory.prototype.CreateJasonfiedGexf = function (meta, pRows, pParent, pChild) {
        ...
        for (var i = 0; i < rows.length; i++) {
            var row = rows[i];
            var parent = row[self.indexOfParent] ? row[self.indexOfParent] : row[meta[self.indexOfParent].name];
            var schemaParent = row[self.indexOfschemaParent] ? row[self.indexOfschemaParent] : row[meta[self.indexOfschemaParent].name];
            var schemaChild = row[self.indexOfschemaChild] ? row[self.indexOfschemaChild]: row[meta[self.indexOfschemaChild].name];
            var child = row[self.indexOfChild] ? row[self.indexOfChild] : row[meta[self.indexOfChild].name];
            var identityNode = row[self.indexOfIdentity] ? row[self.indexOfIdentity] : row[meta[self.indexOfIdentity].name];
            
            nodeFound = graph.nodes.some(function (item) {
                return item.label.toLowerCase() === identityNode.toLowerCase();
            });
            if (!nodeFound) {
                var identity =
 addNode(identityNode, identityNode, i, schemaParent);
            }      ;
